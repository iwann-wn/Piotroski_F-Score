...
$errorMessage = ''; // Variabel untuk menyimpan pesan kesalahan

// Cek jika tidak ada ID yang dikirimkan dari halaman sebelumnya
if (!isset($_GET['id'])) {
    ...
    try {
        ...
        if ($_FILES['file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($fileInfo['extension'], ['xlsx', 'xls'])) {
                ...
                // Loop data dan perbarui data berdasarkan ID
                foreach ($data as $row) {
                    ...
                    //$long_term_debt_score = ($long_term_debt_to_asset_previous > $long_term_debt_to_asset_now) ? 1 : 0;
                    ...
                    // Hitung skor untuk Outstanding Shares
                    ...
                    // Isi quality_of_earning dengan data dari net_income, operating_cashflow, serta skor
                    $quality_of_earning = $net_income . ' / ' . $operating_cashflow . ' / ' . $quality_of_earning_score;

                    // Perhitungan total skor
                    $total_score = $net_income_score + $operating_cashflow_score + $return_on_asset_score + $quality_of_earning_score + $long_term_debt_score + $current_ratio_score + $outstanding_shares_score + $gross_margin_score + $asset_turnover_score;

                    // Update data ke database
                    $sql = "UPDATE stock SET kode_saham=?, tahun=?, net_income=?, operating_cashflow=?, return_on_asset_previous=?, return_on_asset_now=?, quality_of_earning=?, long_term_debt_to_asset_previous=?, long_term_debt_to_asset_now=?, current_ratio_previous=?, current_ratio_now=?, outstanding_shares_previous=?, outstanding_shares_now=?, gross_margin_previous=?, gross_margin_now=?, asset_turnover_previous=?, asset_turnover_now=?, skor=? WHERE id=?";
                ...
                // Arahkan ke halaman index.php jika update berhasil
                header('Location: index.php');
                exit();
            } else {
                // Tipe file tidak valid, berikan pesan kesalahan
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4 mb-4">Halaman Edit </h1>
                            ...
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="mb-3">
                                    <div>
                                        ...
                                        <button type="submit" class="btn btn-outline-dark mt-3" name="upload">Edit
                                            Data</button>
                                    </div>
                            </form>
